...
#define	STRIPS	4
 {.type=REVK_SETTINGS_UNSIGNED,.gpio=1,.name="ledgpio",.comment="GPIOs for LED chains",.group=1,.len=7,.dot=3,.def="4",.ptr=&ledgpio,.size=sizeof(revk_gpio_t),.fix=1,.set=1,.flags="- ~↓↕⇕",.array=STRIPS,.old="rgb"		},
 {.type=REVK_SETTINGS_UNSIGNED,.name="ledcount",.comment="How many LEDs in each chain",.group=1,.len=8,.dot=3,.ptr=&ledcount,.size=sizeof(uint16_t),.array=STRIPS,.old="leds"	},
 {.type=REVK_SETTINGS_UNSIGNED,.isenum=1,.name="ledtype",.comment="Type of LED",.group=1,.len=7,.dot=3,.ptr=&ledtype,.size=sizeof(uint8_t),.array=STRIPS,.enums="WS2812 GRB,WS2812 GBR,WS2812 RGB,WS2812 RBG,WS2812 BGR,WS2812 BRG,WS2812 GRBW,WS2812 GBRW,WS2812 RGBW,WS2812 RBGW,WS2812 BGRW,WS2812 BRGW,SK7812 GRB,SK7812 GBR,SK7812 RGB,SK7812 RBG,SK7812 BGR,SK7812 BRG,SK7812 GRBW,SK7812 GBRW,SK7812 RGBW,SK7812 RBGW,SK7812 BGRW,SK7812 BRGW" },
 ...
 {.type=REVK_SETTINGS_UNSIGNED,.name="maxw",.comment="Max RGBW White",.group=2,.len=4,.dot=3,.def="255",.ptr=&maxw,.size=sizeof(uint8_t)},
 {.type=REVK_SETTINGS_UNSIGNED,.name="webcontrol",.comment="Web controls",.len=10,.def="2",.ptr=&webcontrol,.size=sizeof(uint8_t)},
 {.type=REVK_SETTINGS_UNSIGNED,.gpio=1,.name="lsscl",.comment="Colour sensor SCL",.group=3,.len=5,.dot=2,.ptr=&lsscl,.size=sizeof(revk_gpio_t),.fix=1,.set=1,.flags="- ~↓↕⇕"},
 ...
 {.type=REVK_SETTINGS_UNSIGNED,.gpio=1,.name="micdata",.comment="I2S Mic Data / SD / DATA",.group=4,.len=7,.dot=3,.ptr=&micdata,.size=sizeof(revk_gpio_t),.fix=1,.set=1,.flags="- ~↓↕⇕",.old="audiodata"	},
 {.type=REVK_SETTINGS_UNSIGNED,.gpio=1,.name="micclock",.comment="I2S Mic Clock / BCLK / SCK",.group=4,.len=8,.dot=3,.ptr=&micclock,.size=sizeof(revk_gpio_t),.fix=1,.set=1,.flags="- ~↓↕⇕",.old="audioclock"	},
 {.type=REVK_SETTINGS_UNSIGNED,.gpio=1,.name="micws",.comment="I2S Mic WS / LR (not set for PDM mode)",.group=4,.len=5,.dot=3,.ptr=&micws,.size=sizeof(revk_gpio_t),.fix=1,.set=1,.flags="- ~↓↕⇕",.old="audiows"		},
 ...
 {.type=REVK_SETTINGS_UNSIGNED,.name="micrange",.comment="Range for reactive effects",.group=4,.len=8,.dot=3,.def="10",.ptr=&micrange,.size=sizeof(uint8_t),.live=1,.unit="dB"	},
 {.type=REVK_SETTINGS_BIT,.name="haenable",.comment="Enable Home Assistant",.group=5,.len=8,.dot=2,.def="1",.bit=REVK_SETTINGS_BITFIELD_haenable,.hide=1},
 {.type=REVK_SETTINGS_UNSIGNED,.name="onpower",.comment="Activate specified preset on power up",.group=6,.len=7,.dot=2,.def="1",.ptr=&onpower,.size=sizeof(uint8_t),.hide=1,.live=1,.old="poweron"	},
 {.type=REVK_SETTINGS_UNSIGNED,.name="onclap",.comment="Activate specified preset on loud clap/tap",.group=6,.len=6,.dot=2,.ptr=&onclap,.size=sizeof(uint8_t),.hide=1,.live=1,.old="clapon"	},
 {.type=REVK_SETTINGS_UNSIGNED,.name="onbutton",.comment="Activate specified preset on button press",.group=6,.len=8,.dot=2,.def="1",.ptr=&onbutton,.size=sizeof(uint8_t),.hide=1,.live=1},
