...
Some apps are designed to work on an LED ring - this may be the whole LED strip, in which case you may not need any special settings, but it could also be part of a chain of LEDs with other things. To constrain the working of a ring based app to the ring, these settings are useful. If not set it is assumed the entire chain is one ring.
...
Each app can take a numeric paramater - for some one off apps this is usually number of seconds. In some cases a simple colour is used, 0-31 which is actually 2 bits R, G, B. This is not full RGB options as this is also scaled to `bright` and the colour normally has a fade up.

### Simple apps

- [Idle](idle.md) Simple all LEDs off or specific colour (0-31).
...
- [Ring Idle](ringidle.md) All LEDS in ring off or specific colour (0-31).
